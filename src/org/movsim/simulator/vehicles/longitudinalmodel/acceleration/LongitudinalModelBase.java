...
public abstract class LongitudinalModelBase {
    ...
    public enum ModelName {
        ...
        FB(ModelCategory.TIME_CONTINUOUS_MODEL, "Flocking-Behavior-Model"),
        BMA(ModelCategory.TIME_CONTINUOUS_MODEL, "Bilateral-Multi-Anticipative"), 
        CalibratedIDM(ModelCategory.TIME_CONTINUOUS_MODEL, "Calibrated Intelligent-Driver-Model");
     ...
     * @author gueriau
     */
    protected double s0RandomizationFactor = 1;
     ...
     * 
     * TODO : minimum gap randomization
     * 
    ...
    public double getMinimumGap() {
        return s0RandomizationFactor * getParameter().getS0();
    }
    ...
    /**
     * Sets the relative randomization S0.
     * 
     * <br>
     * Needs to be overwritten if not applicable to model.
     * 
     * @param relRandomizationFactor
     *            the new relative randomization S0
     *            
     * @author gueriau
     */
    public void setRelativeRandomizationS0(double relRandomizationFactor, DistributionTypeEnum distributionType) {
        if (distributionType == DistributionTypeEnum.GAUSSIAN) {
            s0RandomizationFactor = MyRandom.getGaussiansDistributedRandomizedFactor(relRandomizationFactor, 3);
        }else {
            s0RandomizationFactor = MyRandom.getUniformlyDistributedRandomizedFactor(relRandomizationFactor);
        }
        Preconditions.checkArgument(s0RandomizationFactor > 0, "relative S0 randomization factor must be > 0");
        LOG.debug("randomization (of type={}) of desired speeds with randomization factor=", distributionType,
                s0RandomizationFactor);
    }
     ...
     * @return the acceleration of vehicle me
     */
    public double calcAccEurUsingPerception(double vCritEur, Vehicle me, Vehicle frontVehicle, Vehicle leftVehicle,
            double alphaT, double alphaV0, double alphaA) {

    	 // calculate normal acceleration in own lane
        final double accInOwnLane = calcAcc(me, frontVehicle, alphaT, alphaV0, alphaA);
        ...
        final double speedFront = leftVehicle.getSpeed();
        if (speedFront <= vCritEur) {
            return accInOwnLane;
        }
        ...
        // condition me.getSpeed() > speedFront will be evaluated by softer tanh
        // condition below
        final double accLeft = calcAcc(me, leftVehicle, alphaT, alphaV0, alphaA);
