...
import org.movsim.simulator.vehicles.longitudinalmodel.acceleration.BMA;
...
public class TrafficCanvas extends SimulationCanvasBase implements SimulationRunnable.UpdateDrawingCallback,
    ...
    protected boolean drawDetectors = true;
    ...
    public enum VehicleColorMode {
        VELOCITY_COLOR, LANE_CHANGE, ACCELERATION_COLOR, VEHICLE_LABEL_COLOR, VEHICLE_COLOR, COOPERATIVE_COLOR,EXIT_COLOR
    }
   ...
   public void setupTrafficScenarioWithoutResetting(String scenario, String path) {
    ...
    protected Color vehicleColor(Vehicle vehicle, double simulationTime) {
        ...
        case EXIT_COLOR:
            ...
            break;
        case COOPERATIVE_COLOR:
        	if(vehicle.getId() == vehicleToHighlightId)
            color = Color.CYAN;
            else if(vehicle.getLongitudinalModel() instanceof BMA || vehicle.getLabel().compareTo("Coop") == 0 || vehicle.getLabel().compareTo("Connected") == 0){
            	color = Color.BLUE;
            } else {
            	color = Color.RED;
            }
    ...
    protected void drawForeground(Graphics2D g) {
        ...
        synchronized (simulationRunnable.dataLock) {
            ...
            for (final RoadSegment roadSegment : roadNetwork) {
                ...
                /**
                 * Draw RoadSideUnits && detectors
                 */
                for(RoadSideUnit rsu : roadSegment.getRSUs()){
                	drawRSU(g,simulationTime,roadMapping, rsu);
                	for(Sensor s : rsu.getSensors()){
                		
                		if(s instanceof RSULoopDetector){
                			drawDetector(g, roadSegment, roadMapping, (RSULoopDetector) s, rsu);
                		}
                	}
                }
                ...
                if(drawDetectors && roadSegment.getLoopDetectors()!=null){
                	for(LoopDetector detector : roadSegment.getLoopDetectors().getDetectors()){
                		drawDetector(g, roadSegment, roadMapping,detector);
                	}
                }
     ...
     * @param rsu
     */
    private void drawRSU(Graphics2D g, double simulationTime, RoadMapping roadMapping, RoadSideUnit rsu) {
       
    	 final double offset = (roadMapping.laneCount() / 2.0) * roadMapping.laneWidth() + rsu.getOffset();
         final double size = 0.3 * roadMapping.laneWidth();
         final RoadMapping.PosTheta posTheta = roadMapping.map(rsu.getPosition(), offset);
         ...
         g.setColor(rsu.getColor());
         g.fillOval((int) (x - size), (int) (y - size), (int) (2 * size), (int) (2 * size));
      
         
        if(vehicleColorMode == VehicleColorMode.COOPERATIVE_COLOR && vehicleToHighlightId!=-1){
        	   
            final double radius = rsu.getRange();
    ...
    private void drawVehicle(Graphics2D g, double simulationTime, RoadMapping roadMapping, Vehicle vehicle) {
         ...
         * Cooperation mode
         */
        if(vehicleColorMode == VehicleColorMode.COOPERATIVE_COLOR && vehicle.getId() == vehicleToHighlightId && vehicle.getLongitudinalModel() instanceof BMA){
           for(Sensor s : vehicle.getSensors()){
        	   if(s instanceof CommunicationDevice){
        		   final double range = ((CommunicationDevice) s).range;
        		   g.setPaint(Color.cyan);
        		   g.drawOval((int) ((polygon.xPoints[0] + polygon.xPoints[1]) /2.-range), (int) ((polygon.yPoints[0] + polygon.yPoints[2])/2.-range), (int)range*2, (int)range*2);
        		   g.setPaint(Color.white);
        		   g.drawRect((int) ((polygon.xPoints[0] + polygon.xPoints[1]) /2. + vehicle.getVirtualVehicle().getRelativePosition()) , (int) ((polygon.yPoints[0] + polygon.yPoints[2])/2.) -2 , 6, 4);
        	   }
           }
        	
           MeasurementPerception p = (MeasurementPerception) vehicle.getPerception();
           for(SensedVehicle v : p.getSurroundingVehicles()){
        	   g.setPaint(Color.green);
        	   g.drawRect((int) ((polygon.xPoints[0] + polygon.xPoints[1]) /2. + v.getRelativePosition()) , (int) ((polygon.yPoints[0] + polygon.yPoints[2])/2.) -2 , 1, 4);
           }
    ...
    /**
     * Draws the loop detectors
     * 
     * @param g
     * @param detector2 
     * @param roadMapping 
     */
    private void drawDetector(Graphics2D g, RoadSegment roadSegment, RoadMapping roadMapping, LoopDetector detector){
    	
    	for(int i = 0; i<roadSegment.laneCount(); ++i){
    	
	    	final RoadMapping.PosTheta posTheta = roadMapping.map(detector.getDetPosition(),roadMapping.laneOffset(i));
	    	final RoadMapping.PolygonFloat polygon = roadMapping.mapFloat(posTheta, 1, roadMapping.laneWidth());
	    	
	    	g.setPaint(Color.darkGray);
	  	   	g.drawRect((int)(-2+polygon.xPoints[0]),(int) (polygon.yPoints[0]+0.1*roadMapping.laneWidth()), 2, (int) (0.8*roadMapping.laneWidth()));
	  	   	g.drawRect((int) (polygon.xPoints[0]),(int) (polygon.yPoints[0]+0.1*roadMapping.laneWidth()), 2, (int) (0.8*roadMapping.laneWidth()));
    	
    	}
    }
    
    private void drawDetector(Graphics2D g, RoadSegment roadSegment, RoadMapping roadMapping, RSULoopDetector detector, RoadSideUnit rsu) {
    	for(int i = 0; i<roadSegment.laneCount(); ++i){
        	
	    	final RoadMapping.PosTheta posTheta = roadMapping.map(rsu.getPosition(),roadMapping.laneOffset(i));
	    	final RoadMapping.PolygonFloat polygon = roadMapping.mapFloat(posTheta, 1, roadMapping.laneWidth());
    ...
    private static void drawTrafficLightsOnRoad(Graphics2D g, RoadSegment roadSegment) {
        ...
        for (TrafficLightLocation trafficLightLocation : roadSegment.trafficLightLocations()) {
            if(trafficLightLocation.getTrafficLight()!=null){
