...
import fr.ifsttar.licit.simulator.agents.trust.representation.LocalTrust;
...
public abstract class Vehicle extends Agent {
	public static int WAITING_TIME_VALUE = 15;
	private int waitingTime = WAITING_TIME_VALUE;
	...
	public void initWaitingTime(){
		waitingTime = WAITING_TIME_VALUE ;
	}
	...
	protected LongitudinalModelBase longitudinalModel;
	/** can be null */
	 ...
	 * 
	 */
	private double speedRecommendation = -1;
	public int closedLaneAhead = 0;

	/**
	 * Flocking
	 */
	private double sharedSpeedSum = 0;
	private double sharedSpeedNum = 0;

	/**
	 * Group disagreement value
	 */
	private double gd = 0.0;

	/** cheat **/
	protected long frontVehicleId;
	
	/** add for tp
	 * 
	 * 
	 */
	private HashMap<Long, SensedVehicle> communicatingVehicles = new HashMap<Long,SensedVehicle>();
	
	public void setFrontVehicleId(Vehicle v) {
		if (v != null) {
			this.frontVehicleId = v.getId();
		} else {
			this.frontVehicleId = 0;
		}
	}

	/**
	 * Must be called only once a timestep (reset the value)
	 * 
	 * @return the mean velocity computed from received messages
	 */
	public double getSharedSpeed() {

		double speed = this.getSpeedlimit();
		if (sharedSpeedNum > 0) {
			speed = sharedSpeedSum / sharedSpeedNum;
		}
	...
	public Vehicle(double rearPosition, double speed, int lane, double length,
		...
		this.trust = new LocalTrust(id);
		this.sensors.clear();
		this.perception = new VehiclePerception();
	...
	public void updateAcceleration(double dt, RoadSegment roadSegment,
		...
		accModel = calcAccModel(laneSegment, leftLaneSegment, alphaTLocal,
				alphaV0Local, alphaALocal);

		// moderate acceleration by traffic lights or for preparing mandatory
		// lane changes to exit sliproads
	 ...
	 * @author gueriau
	 */
	public void updateAccelerationUsingPerception(double dt,
			RoadSegment roadSegment, LaneSegment laneSegment,
		...
		if (noise != null) {
			...
			accError = noise.getAccError();
			final Vehicle frontVehicle = laneSegment.frontVehicle(this);
			// final Vehicle frontVehicle = ((VehiclePerception)
			// perception).getFrontVehicle();
			if (getNetDistance(frontVehicle) < MovsimConstants.CRITICAL_GAP) {
				accError = Math.min(accError, 0.); // !!!
			}
		}
		...
		// accModel = calcAccModel(laneSegment, leftLaneSegment, alphaTLocal,
		// alphaV0Local, alphaALocal);
		// accModel = calcAccModelUsingPerception(((VehiclePerception)
		// perception).getFrontVehicle(), ((VehiclePerception)
		// perception).getLeftVehicle(), alphaTLocal, alphaV0Local,
		// alphaALocal);

		if (leftLaneSegment != null)
			accModel = calcAccModelUsingPerception(
					laneSegment.frontVehicle(this),
					leftLaneSegment.frontVehicle(this), alphaTLocal,
					alphaV0Local, alphaALocal);
		else
			accModel = calcAccModelUsingPerception(
					laneSegment.frontVehicle(this), null, alphaTLocal,
					alphaV0Local, alphaALocal);
		// moderate acceleration by traffic lights or for preparing mandatory
		// lane changes to exit sliproads
		acc = moderateAcceleration(accModel, roadSegment);
	...
	private ArrayList<Long> getSensedIds(
			ArrayList<SensedVehicle> surroundingVehicles) {

		ArrayList<Long> sensedIds = new ArrayList<Long>();

		for (SensedVehicle vehicle : surroundingVehicles) {
			if (vehicle.getSenderId() != this.getId())
				sensedIds.add(vehicle.getSenderId());
		}
	 ...
	 * tactical considerations (say, the desire to make a lane change).
	 * 
	 * @param acc
	...
	protected double accelerationConsideringTrafficLight(double acc,
			RoadSegment roadSegment) {
		double moderatedAcc = acc;
		TrafficLightLocationWithDistance location = roadSegment
				.getNextDownstreamTrafficLight(getFrontPosition(), lane(),
						TrafficLightApproaching.MAX_LOOK_AHEAD_DISTANCE);
		if (location != null) {
			LOG.debug("consider trafficlight={}", location.toString());
			updateTrafficLightApproaching(location.trafficLightLocation,
					location.distance);
			if (trafficLightApproaching.considerTrafficLight()) {
				moderatedAcc = Math.min(acc,
						trafficLightApproaching.accApproaching());
			}
		}
		return moderatedAcc;
	}

	private void updateTrafficLightApproaching(
			TrafficLightLocation trafficLightLocation, double distance) {
		assert distance >= 0 : "distance=" + distance;
		trafficLightApproaching.update(this,
				trafficLightLocation.getTrafficLight(), distance);
	}
	...
	protected double accelerationConsideringExit(double acc,
		...
		if (exitRoadSegmentId == roadSegment.id() && lane() != Lanes.LANE1) {
			...
			// TODO this reaction is in same situations too short-sighted so
			// that the vehicle in the exit lane must be
			// considered already in the upstream segment
			final LaneSegment exitLaneSegment = roadSegment
					.laneSegment(roadSegment.trafficLaneMax());
			if (exitLaneSegment != null
					&& exitLaneSegment.type() == Lanes.Type.EXIT) {
				// this front vehicle could also result in negative net
				// distances
				// but the deceleration is limited anyway
				Vehicle frontVehicle = exitLaneSegment.frontVehicle(this);
				double accToVehicleInExitLane = longitudinalModel.calcAcc(this,
						frontVehicle);
	 ...
	 * 
	 */
	private double calcAccModelUsingPerception(Vehicle frontVehicle,
			Vehicle leftVehicle, double alphaTLocal, double alphaV0Local,
			double alphaALocal) {
		if (longitudinalModel == null) {
			return 0.0;
		}
		...
		if (laneChangeModel != null && laneChangeModel.isInitialized()
			...
			if (longitudinalModel instanceof BasedOnVehiclePerceptionModel) {
				BasedOnVehiclePerceptionModel basedOnPerceptionModel = (BasedOnVehiclePerceptionModel) longitudinalModel;
				acc = basedOnPerceptionModel.calcAccEurUsingPerception(this,
						(VehiclePerception) this.perception, this.trust,
						alphaTLocal, alphaV0Local, alphaALocal);
			} else if (longitudinalModel instanceof BasedOnMeasurementPerceptionModel) {
				BasedOnMeasurementPerceptionModel basedOnMeasurementPerceptionModel = (BasedOnMeasurementPerceptionModel) longitudinalModel;
				// System.out.println(longitudinalModel.getClass());
				acc = basedOnMeasurementPerceptionModel
						.calcAccEurUsingPerception(this,
								(MeasurementPerception) this.perception,
								this.trust, alphaTLocal, alphaV0Local,
								alphaALocal);
			} else {
				acc = longitudinalModel.calcAccEurUsingPerception(
						laneChangeModel.vCritEurRules(), this, frontVehicle,
						leftVehicle, alphaTLocal, alphaV0Local, alphaALocal);
			}
		} else {
			if (longitudinalModel instanceof BasedOnVehiclePerceptionModel) {
				BasedOnVehiclePerceptionModel basedOnPerceptionModel = (BasedOnVehiclePerceptionModel) longitudinalModel;
				// FIXME: implements these methods !
				acc = basedOnPerceptionModel.calcAccUsingPerception(this,
						(VehiclePerception) this.perception, this.trust);
			} else if (longitudinalModel instanceof BasedOnMeasurementPerceptionModel) {
				BasedOnMeasurementPerceptionModel basedOnMeasurementPerceptionModel = (BasedOnMeasurementPerceptionModel) longitudinalModel;
				acc = basedOnMeasurementPerceptionModel.calcAccUsingPerception(
						this, (MeasurementPerception) this.perception,
						this.trust);
			} else {
				acc = longitudinalModel.calcAcc(this, frontVehicle,
						alphaTLocal, alphaV0Local, alphaALocal);
			}
		}
	...
	public boolean considerLaneChange(double dt, RoadSegment roadSegment) {
		 ...
		 * 
		 */
		// LaneChangeDecision lcDecision =
		// laneChangeModel.makeDecision(roadSegment);
		/**
		 * 
		 */
		LaneChangeDecision lcDecision;
		if (closedLaneAhead > 0) {
			lcDecision = laneChangeModel.makeDecision(roadSegment,
					closedLaneAhead);
		} else {
			lcDecision = laneChangeModel.makeDecision(roadSegment);
		}
		...
		if (route != null && routeIndex < route.size()) {
			...
			if (routeRoadSegment.id() == roadSegmentId) {
				...
				if (routeIndex < route.size()) {
					...
					final RoadSegment nextRouteRoadSegment = route
							.get(routeIndex);
					if (routeRoadSegment.exitsOnto(nextRouteRoadSegment.id())) {
						...
						if (routeIndex + 1 < route.size()) {
							// there is another roadSegment on the route
							// so check if the next roadSegment is joined to an
							// exit lane
							// of the current roadSegment
							final RoadSegment nextNextRouteRoadSegment = route
									.get(routeIndex + 1);
							if (nextRouteRoadSegment
									.exitsOnto(nextNextRouteRoadSegment.id())) {
								// this vehicle needs to exit on this
								// roadSegment
								exitRoadSegmentId = roadSegmentId;
							}
						}
	...
	public String toString() {
		return "Vehicle [label=" + label + ", length=" + length
				+ ", frontPosition=" + frontPosition + ", frontPositionOld="
				+ frontPositionOld + ", speed=" + speed + ", accModel="
				+ accModel + ", acc=" + acc + ", accOld=" + accOld + ", id="
				+ id + ", vehNumber=" + vehNumber + ", lane=" + lane + "]";
	}
	...
	/**
	 * Reset the perception information of this Vehicle
	 * 
	 ...
	 */
	/*
	 * public void addPerception(VehiclePerception p) { super.addPerception(p);
	 * }
	 */

	/**
	 * Adds self parameters
	 * 
	 * @author Maxime Guériau
	 */
	@Override
	...
	public double getSpeedRecommandation() {
		return this.speedRecommendation;
	}

	protected SensedVehicle virtualVehicle = new SensedVehicle(0, 0, 0, 0, 0,0, 0);

	public SensedVehicle getVirtualVehicle() {
		return this.virtualVehicle;
	}

	public void setVirtualVehicle(SensedVehicle sensedVehicle) {
		this.virtualVehicle = sensedVehicle;
	}

	public void cheat(double velocity, double acceleration) {
	...
	public void setGd(double value) {
		this.gd = value;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see fr.ifsttar.licit.simulator.agents.Agent#getInitialPerception()
	 */
	@Override
	...
	public HashMap<Long, SensedVehicle> getCommunicatingVehicles() {
