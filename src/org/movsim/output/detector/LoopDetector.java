...
public class LoopDetector implements SimulationTimeStep {
     ...
     * @param loggingLanes
     */
    public LoopDetector(RoadSegment roadSegment, int id, double detPosition, double dtSample, boolean logging, boolean loggingLanes) {
    ...
    public ArrayList<Double> getFlows(){
    	...
    	for(int i =0; i< this.laneCount; ++i)
    		flows.add(vehCountOutput[i] / dtSample);
    	return flows;
    	
    }
    
    public ArrayList<Double> getMeanSpeeds(){
    	ArrayList<Double> meanSpeeds = new ArrayList<Double>();
    	for(int i =0; i< this.laneCount; ++i)
    		meanSpeeds.add(getMeanSpeed(i));
    	return meanSpeeds;
