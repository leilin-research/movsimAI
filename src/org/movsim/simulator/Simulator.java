...
import org.movsim.autogen.ExternalControl;
...
public class Simulator implements SimulationTimeStep, SimulationRun.CompletionCallback {
     ...
     * 
     */
    public static Server externalControlServer;
     ...
     *
     */
    /*private SpaceTimeSpeedDataSet spaceTimeSpeedDataSet;
    
    

    public SpaceTimeSpeedDataSet getSpaceTimeSpeedDataSet() {
    	if(spaceTimeSpeedDataSet==null){
    		
    	}
		return spaceTimeSpeedDataSet;
	}
	*/
	/**
	 * 
    ...
    public void initialize() throws JAXBException, SAXException {
        ...
        vehicleFactory = new VehicleFactory(this,simulationInput.getTimestep(), inputData.getVehiclePrototypes(),
        ...
        if(simulationInput.isSetExternalControl()) { //check from config and get ports 
        	...
        	ExternalControl externalControlParameters = simulationInput.getExternalControl();
        	
        	List<RoadSideUnit> rsus = new ArrayList<RoadSideUnit>();
        	for(RoadSegment roadSegment : this.roadNetwork){
        		//System.out.println(roadSegment);
        		rsus.addAll(roadSegment.getRSUs());
        	}
        	try {
				this.externalControlServer = new Server(externalControlParameters.getAgentPort(), externalControlParameters.getBackLogPort(), rsus);// new Server("2015-complete-3sims.xml", rsus);
				this.externalControlServer.run();
    ...
    private void addInputToRoadSegment(RoadSegment roadSegment, Road roadInput) {
         ...
         */
        //set up the RSUs
        if(roadInput.isSetRoadSideUnits()){
        	roadSegment.setRoadSideUnits(roadInput.getRoadSideunits().getRoadSideUnit(), roadNetwork);
        }
        /**
         * 
         */
    ...
    public void simulationComplete(double simulationTime) {
    	...
    	if(this.externalControlServer!=null){
    		this.externalControlServer.stopSimulation();
    	}
