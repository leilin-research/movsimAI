...
/**
 * <p>
 * Class to encapsulate a simulation thread. Includes the necessary
 * synchronization and callbacks to coordinate with an application UI thread.
 * </p>
 * 
 * <p>
 * This class is generic and can be used for any type of simulation: it can be
 * used with any simulation object that implements the SimulationTimeStep
 * interface.
 * </p>
 * 
 */
public class SimulationRunnable extends SimulationRun implements Runnable {
	...
	public interface UpdateDrawingCallback {
		/**
		 * Callback to get the application to do a draw after the vehicles have had
	...
	public interface HandleExceptionCallback {
		/**
		 * Callback to allow the application to handle an exception in the main run()
		 * loop.
		 * 
		 * @param e
		 */
	 ...
	 * new version
	 */
	// private ArrayList<UpdateDrawingCallback> updateDrawingCallbacks = new
	// ArrayList<UpdateDrawingCallback>();
	...
	private static final int DEFAULT_SLEEP_TIME_MS = 10; // default sleep time, milliseconds
	 ...
	 */
	/*
	 * public void addUpdateDrawingCallback(UpdateDrawingCallback
	 * updateDrawingCallback) { //assert this.updateDrawingCallback == null; // it's
	 * a mistake if this is set twice
	 * 
	 * //System.out.println("add call back");
	 * //System.out.println(updateDrawingCallback);
	 * this.updateDrawingCallbacks.add(updateDrawingCallback); } /** needed counter
	 * method
	 */
	/*
	 * public void removeUpdateDrawingCallback(UpdateDrawingCallback
	 * updateDrawingCallback) { //assert this.updateDrawingCallback == null; // it's
	 * a mistake if this is set twice
	 * this.updateDrawingCallbacks.remove(updateDrawingCallback); }
	 */
	...
	/**
	 * Pauses the simulation thread. Differs from stop in that the running state is
	 * saved so that it can be restored on resume.
	 */
	public void pause() {
		if (thread != null) {
			stop();
	...
	/**
	 * Resumes the simulation thread. Change back to the running state we had before
	 * pause was called.
	 */
	public void resume() {
	 ...
	 * <code>timeStep(dt, simulationTime, iterationCount)</code> is called for the
	 * simulation object. Typically the simulation object is an iterable collection
	 * of elements, each with their own timestep method.
	 * </p>
	 * 
	 * <p>
	 * This method must be synchronized (using <code>dataLock</code>) (normally with
	 * the <code>drawForeground</code> method), so that elements are not updated,
	 * added or removed from the simulation while they are being drawn.
	 * </p>
	 * 
	 * <p>
	 * <code>updateStatus(simulationTime)</code> is called after the simulation
	 * object has been updated to allow the application to make updates before the
	 * repaint is called.
	 * </p>
	 * 
	 */
	...
	public void run() {
		...
		while (Thread.currentThread() == thread) {
			 ...
			 * new version
			 */
			// for(UpdateDrawingCallback updateDrawingCallback :
			// this.updateDrawingCallbacks){
	...
	public void runToCompletion() {
		...
		while (simulationTime <= timeLimit) {
			...
			// System.out.println(this.updateDrawingCallbacks.size());
	...
	private void calculateTimewarp() {
		...
		smoothedTimewarp = Math.min(1000, smoothedTimewarp == 0.0 ? actualTimewarp
