...
import org.movsim.output.trust.FileTrust;
...
public class SimulationOutput implements SimulationTimeStep {
    ...
    private final Map<Route, FileTrust> filesTrust = new HashMap<>();
    ...
    private final Map<Route, FileTravelTimes> filesTravelTimes = new HashMap<>();
     ...
     * 
     */
    public ArrayList<FileTrajectories> getTrajectories(){
    	return new ArrayList<FileTrajectories>(this.filesTrajectories.values());
    }
    /**
     *
     */
    ...
    private void initTravelTimes(boolean writeOutput, final OutputConfiguration outputConfiguration) {
        if (writeOutput) {
        	
        	//System.out.println(" <<<<<<<<<<<<<<<<<<<<<<< OPEN SPATIO TEMPORAL VIEW " + outputConfiguration.getTrajectories().size());
            for (final TravelTimes tt : outputConfiguration.getTravelTimes()) {
                final Route route = getCheckedRoute(tt.getRoute());
                if (filesTravelTimes.containsKey(route)) {
                    logger.warn("travel time output for route \"{}\" already defined!", route.getName());
                    continue;
                }
                filesTravelTimes.put(route, new FileTravelTimes(tt, route));
            }
        }
    }
    ...
    private void initTrusts(boolean writeOutput, final OutputConfiguration outputConfiguration) {
        if (writeOutput) {
        	
        	//System.out.println(" <<<<<<<<<<<<<<<<<<<<<<< OPEN SPATIO TEMPORAL VIEW " + outputConfiguration.getTrajectories().size());
            for (final Trusts trusts : outputConfiguration.getTrusts()) {
                final Route route = getCheckedRoute(trusts.getRoute());
                if (filesTrust.containsKey(route)) {
                    logger.warn("trust output for route \"{}\" already defined!", route.getName());
                    continue;
                }
                filesTrust.put(route, new FileTrust(trusts, route));
            }
        }
    }
    ...
    public void timeStep(double dt, double simulationTime, long iterationCount) {
        ...
        for (final FileTravelTimes travelTime : filesTravelTimes.values()) {
            travelTime.timeStep(dt, simulationTime, iterationCount);
        }
        
        for (final FileTrust fileTrust : filesTrust.values()) {
        	fileTrust.timeStep(dt, simulationTime, iterationCount);
        }
